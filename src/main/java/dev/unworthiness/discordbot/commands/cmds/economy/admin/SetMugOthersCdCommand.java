...
public class SetMugOthersCdCommand implements ICommand {
  ...
  public void handle(CommandContext ctx) {
    ...
    updateMuggingCd(ctx.getGuild().getIdLong(), newCd);

    channel.sendMessageFormat("Mugging cooldown has been set to `%s`", newCd).queue();
  }
  ...
  public String getHelp() {
    return "Sets cooldown, in seconds, for how often users can mug other users";
  }

  private void updateMuggingCd(long guildId, long cd) {
    EconomyHandler.MUG_OTHER_COOLDOWNS.put(guildId, cd);
    ...
    try (PreparedStatement statement =
        connection
            // language=SQLite
            .prepareStatement("UPDATE guild_settings SET mug_other_cd = ? WHERE guild_id = ?")) {
